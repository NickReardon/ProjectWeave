...
# 19 — Note Structure and Dogfood Vault
...
Roadmap order is topological, then rank, then normalized path. Duplicate ranks
warn and missing ranks sort after ranked peers. Epic milestone membership is
derived from the Epic link; task milestone membership remains independently
authored and is never inherited.

Milestone `due_date` is optional. Due and overdue state is derived only when a

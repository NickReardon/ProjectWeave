...
Portfolio-scoped planning periods sit outside the single-project v1 core. The portfolio-sprint behavior below remains the specified contract for that scope; it is deferred rather than redesigned, and the same deferral applies as in [Non-goals and future features](non-goals-and-future-features.md).

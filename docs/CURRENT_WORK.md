...
- **State:** the persistent Project Workbench slice is merged on `main` and
  ...
  the deterministic task-template rendering foundation as pure domain code with
  no caller. The branch is unmerged and unreleased; the working tree is clean.
...
**Task-template rendering foundation** (`feat/task-template-renderer`): Plan

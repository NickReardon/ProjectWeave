# 16 — Streamlined Long-Project Workflow
...
The filename supplies title when `title` is missing. The canonical statuses are `backlog`, `todo`, `in-progress`, `waiting`, `review`, `done`, and `cancelled`.

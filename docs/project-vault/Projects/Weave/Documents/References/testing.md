...
4. Break the vault template — change `template_for` to `epic` — reopen, and
   select `bug`.
5. Remove `bug.md`, leaving only the packaged default, and reopen.

**Pass:** the chooser lists `default`, `bug`, and **Built-in default**; the
preview follows the selection each time; the broken template shows `template.kind_mismatch`
